...
const ESPN_V050_CAPTURE_RETRY_MS = 1000;
const ESPN_V050_CAPTURE_RETRY_LIMIT = 12;
...
async function showEspnStatus(sanctumTabId, type, message) {
  ...
  try {
    await extensionApi.scripting.executeScript({ target: { tabId: sanctumTabId }, world: "MAIN", func: function (statusType, statusMessage) {
  ...
  } catch (err) { console.warn("Could not display ESPN connection status.", err); }
}

async function deliverEspnToSanctum(sanctumTabId, captured) {
  if (!(await isEspnSanctumTabStillValid(sanctumTabId))) throw new Error("The Inner Sanctum connection page is no longer open. Return to Link Your League and try again.");
  return extensionApi.scripting.executeScript({ target: { tabId: sanctumTabId }, world: "MAIN", func: function (payload) {
    if (!payload || !payload.league || !payload.team || !Array.isArray(payload.roster) || !payload.roster.length) throw new Error("Incomplete ESPN league data was received.");
    const connectionData = { leagueId: payload.league.id, leagueName: payload.league.name, season: payload.league.season, teamCount: payload.league.teamCount, teamId: payload.team.id, teamName: payload.team.name, private: true, connectionMode: "browser-assisted", readOnly: true, league: payload.league, team: payload.team, roster: payload.roster, standings: payload.standings || [], schedule: payload.schedule || [], matchup: payload.matchup || null, settings: payload.settings || null, meta: payload.meta || null, syncedAt: new Date().toISOString() };
    const scoringFormat = payload?.settings?.scoringProfile?.format || payload?.meta?.dataQuality?.scoringFormat || null;
    if (scoringFormat) connectionData.scoringFormat = scoringFormat;
    if (!window.LeagueConnection || typeof window.LeagueConnection.connect !== "function") throw new Error("The Inner Sanctum league connection manager is unavailable.");
    const alreadyConnected = typeof window.LeagueConnection.isConnected === "function" && window.LeagueConnection.isConnected("espn");
    if (alreadyConnected && typeof window.LeagueConnection.update === "function") window.LeagueConnection.update("espn", connectionData); else window.LeagueConnection.connect("espn", connectionData);
    ...
    const box = document.getElementById("espnResult");
    if (box) { box.className = "result-box success show"; box.textContent = "✓ " + (connectionData.leagueName || "ESPN league") + " connected — " + (connectionData.teamName || "team identified") + "."; }
...
async function sendEspnCapture(tabId, attempt) {
  const n = attempt || 1;
  try { return await extensionApi.tabs.sendMessage(tabId, { type: "INNER_SANCTUM_ESPN_CAPTURE" }); }
  catch (err) {
...
async function captureEspnFromLeagueTab(espnTab, sanctumTabId) {
  if (!espnTab?.id || !ESPN_V050_URL_PATTERN.test(espnTab.url || "")) throw new Error("ESPN has not reached a fantasy-football page yet.");
  let leagueId = "";
  try { leagueId = new URL(espnTab.url).searchParams.get("leagueId") || ""; } catch (err) {}
  if (!leagueId) throw new Error("Open the ESPN league you want to connect.");
  await showEspnStatus(sanctumTabId, "loading", "🔴 ESPN league detected. Syncing your league, team and roster...");
  const response = await sendEspnCapture(espnTab.id, 1);
  if (!response || response.success !== true) throw new Error(response?.error || "ESPN league capture is not ready yet.");
  const captured = response.data;
  if (!captured?.league?.id || !captured?.team?.id || !Array.isArray(captured.roster) || !captured.roster.length) throw new Error("ESPN league data is still incomplete.");
  if (captured.meta?.dataQuality?.complete === false) throw new Error("ESPN league data is still loading.");
  await deliverEspnToSanctum(sanctumTabId, captured);
  return { success: true, leagueName: captured.league?.name || "", teamName: captured.team?.name || "" };
}
...
async function findOpenEspnLeagueTab() {
  ...
  const espnTabs = tabs.filter(function (tab) { return typeof tab.url === "string" && ESPN_V050_URL_PATTERN.test(tab.url); });
  const withLeague = espnTabs.find(function (tab) { try { return Boolean(new URL(tab.url).searchParams.get("leagueId")); } catch (err) { return false; } });
...
async function beginEspnConnect(sender) {
  ...
  if (!sanctumTab || !ESPN_V050_SANCTUM_URL_PATTERN.test(sanctumTab.url || "")) throw new Error("ESPN connection request did not originate from The Inner Sanctum.");
  const existing = await findOpenEspnLeagueTab();
  if (existing) {
    try { return await captureEspnFromLeagueTab(existing, sanctumTab.id); }
    catch (err) {
      await setPendingEspn({ sanctumTabId: sanctumTab.id, providerTabId: existing.id, startedAt: Date.now() });
      await extensionApi.tabs.update(existing.id, { active: true });
      await showEspnStatus(sanctumTab.id, "loading", "🔴 ESPN is open. Sign in if needed and open the league you want — Inner Sanctum will connect automatically.");
      if (new URL(existing.url).searchParams.get("leagueId")) void retryPendingEspnCapture(existing.id, sanctumTab.id);
  ...
  await setPendingEspn({ sanctumTabId: sanctumTab.id, providerTabId: tab.id, startedAt: Date.now() });
  await showEspnStatus(sanctumTab.id, "loading", "🔴 ESPN opened. Sign in normally and open the league you want — Inner Sanctum will detect it automatically.");
  return { success: true, pending: true };
}
...
extensionApi.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  beginEspnConnect(sender).then(function (result) { sendResponse(result); }).catch(async function (err) {
    if (sender.tab?.id) await showEspnStatus(sender.tab.id, "error", "⚠️ " + (err.message || "ESPN connection failed."));
...
extensionApi.tabs.onUpdated.addListener(async function (tabId, changeInfo, tab) {
  ...
  if (Date.now() - Number(pending.startedAt || 0) > ESPN_V050_CONNECT_TIMEOUT_MS) {
    await clearPendingEspn();
    await showEspnStatus(pending.sanctumTabId, "error", "⚠️ ESPN sign-in timed out. Click Connect ESPN League and try again.");
    return;
  }
