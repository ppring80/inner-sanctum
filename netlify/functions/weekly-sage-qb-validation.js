// netlify/functions/weekly-sage-qb-validation.js
//
// WEEKLY SAGE — QB HISTORICAL VALIDATION
//
// PURPOSE
// -------
// Compare the PRE-GAME Weekly SAGE QB forecast against what
...
//
//   Prediction:
//     weekly-sage-qb-leaderboard?week=8
...
// -----------------
//
// For each QB who actually played:
//
//   playerID
//   name
//   team
...
//
// BYE
//   Already excluded from weekly-sage-qb-leaderboard.
//
// DID NOT PLAY
...
/*
  ...
  metadata only. Player-season evidence is produced by an
  in-process call rather than an HTTP self-fetch.
*/
...
/*
  ...
  (buildPlayerSeason) and shared schedule-map builder
  (buildPriorWeekScheduleMap) are required directly, in-process.

  The prior-week schedule map is built exactly once for the entire
  validation run and passed by reference into every player-season
  outcome build.
*/
const {
...
async function fetchLeaderboard({
  ...
  if (
    ...
    throw new Error(
      "Unexpected Weekly SAGE QB leaderboard schema."
    );
  }
...
/*
  ...
  The shared scheduleContext is built exactly once for the
  whole validation run and reused by reference.
*/
...
function actualStats(
  ...
  /*
    Receiving is retained because a QB can technically record
    receiving production on a trick play.

    It is not part of the QB prediction model, but if it occurs in
    the actual target-week result it should count toward actual
    fantasy scoring.
  */
  const targets =
    num(
...
/*
  Generic fantasy validation scoring.
  ...
  Half-PPR and PPR only differ if a QB records an actual reception.

  Fumbles and league-specific bonuses are intentionally excluded.
*/
...
/*
  ...
  so a positive correlation means the rankings broadly agree.
*/
    ...
    */
    ...
    try {
      ...
      /*
        STEP 1
        ------
        Retrieve the frozen PRE-GAME Weekly SAGE QB leaderboard.
      */
      const leaderboard =
      ...
      if (
        ...
        return jsonResponse(
          ...
          {
            error:
              "Weekly SAGE QB leaderboard contains no active QBs."
          }
        );
      }
      ...
      */
      if (
        ...
        return jsonResponse(
          ...
          {
            error:
              "Weekly SAGE QB leaderboard is not clean enough for historical validation.",
      ...
      /*
        STEP 1.5
        --------
        Build the shared prior-week schedule map ONCE.

        Player-season targetWeek is validation targetWeek + 1,
        so this shared map includes schedules through the completed
        validation target week.
      */
      ...
      /*
        ...
        Each player reuses the same scheduleContext.
      */
      ...
      for (
        ...
        /*
          Leaderboard says the player's historical team was ACTIVE.
      ...
      */
      ...
      /*
        Preserve original pre-game SAGE ordering.
      */
      validation.sort(
        function (
          a,
      ...
      */
      ...
      return jsonResponse(
        200,
        {
          ...
          methodology: {
            ...
            prediction:
              `Weekly SAGE QB Week ${targetWeek} scores generated using only evidence available before Week ${targetWeek}.`,
            ...
            participationHandling:
              "Leaderboard bye players remain excluded. Active-team QBs with a target-week player-game record are classified as played. Active-team QBs with no target-week player-game record are classified as Did Not Play and are never assigned zero fantasy points.",
            ...
            componentValidation:
              "Role, Production, and Matchup are correlated independently against actual QB fantasy points so their relative predictive signal can be inspected before weight sensitivity testing.",

            topCapture:
              "Predicted Top-6/12/24 sets are frozen from the original pre-game leaderboard. Actual Top-6/12/24 sets are calculated from QBs who actually played. DNP players are not removed from the frozen predicted sets after the fact.",
            ...
            important:
              "This endpoint measures the untouched provisional QB SAGE v1 model (55/40/5 placeholder). It does not optimize weights or create recommendation thresholds."
          },
          ...
          nextStep: {
            ...
            reason:
              ready
                ? "The frozen QB SAGE forecast has been matched to actual target-week outcomes. Review overall correlations, component correlations, rank agreement, Top-6/12/24 capture, and score-band behavior before changing QB weights."
                : "Resolve outcome retrieval failures before interpreting QB forecast-vs-actual validation."
          },
      ...
      console.error(
        "weekly-sage-qb-validation failed:",
        error
      );
      ...
      return jsonResponse(
        ...
        {
          error:
            "Could not validate Weekly SAGE QB scores against actual outcomes.",
