// netlify/functions/weekly-sage-te-benchmarks.js
//
// WEEKLY SAGE — TE BENCHMARKS
//
// PURPOSE
// -------
// Convert the reusable TE population snapshot into peer-relative
// benchmark evidence for one requested tight end.
//
// SOURCE
// ------
//
...
// - calculate final TE Role or Production scores
...
//
// This endpoint tells us WHERE the requested TE sits relative
// to the eligible TE peer population.
//
// PERCENTILE METHOD
// -----------------
// Midrank:
...
/*
  All raw TE evidence currently available from the snapshot.
...
async function fetchSnapshot({
  ...
  if (
    ...
    throw new Error(
      "Unexpected Weekly SAGE TE snapshot schema."
    );
  }
...
exports.handler =
  async function (
    ...
    try {
      ...
      console.error(
        "weekly-sage-te-benchmarks failed:",
        error
      );
      ...
      return jsonResponse(
        ...
        {
          error:
            "Could not build Weekly SAGE TE benchmarks.",
...
/*
  Core Weekly SAGE TE benchmark computation, extracted additively.
  ...
  response) and may be called by weekly-sage-te-component-scores.js
  ...
  down from weekly-sage-te-leaderboard.js's own single snapshot
  build through component-scores/confidence/final-score, once that
  file exists in a later phase), fetchSnapshot() is never called.
  When omitted, this function fetches it itself exactly as it always
  has, unconditionally, for every existing GET caller and for
  independent GET requests to this endpoint specifically.
*/
async function buildTeBenchmarks({
  baseUrl,
  season,
  targetWeek,
  seasonType,
  ...
  prebuiltSnapshot
      ...
      /*
        STEP 1
        ------
        Fetch the reusable TE population.
      ...
      */
      ...
      if (
        ...
        throw statusError(
          ...
          {
            error:
              "TE snapshot is not ready for benchmark use.",
      ...
      if (
        ...
        throw statusError(
          ...
          {
            error:
              "TE snapshot contains no eligible peer population."
          }
        );
      }
      ...
      /*
        STEP 2
        ------
        Find the requested TE inside the already-validated snapshot.
      ...
      return {
          ...
          methodology: {
            ...
            populationDefinition:
              "Wide receivers meeting the TE snapshot's minimum prior-game and target-volume eligibility rules entering the target week.",
            ...
            direction:
              "Higher percentile means the player's observed value is higher than more of the eligible TE peer population.",
            ...
            important:
              "These percentiles describe the TE peer population. They are not yet final SAGE Role or Production scores and no component weights are applied."
          },
          ...
          nextStep: {
            ...
            reason:
              "TE benchmark percentiles are available. Inspect representative TE profiles before selecting the evidence and weights used to construct TE Role and Production component scores."
          },
