...
exports.handler = async (event) => {
  ...
  if (event.httpMethod !== "GET") {
    return { statusCode: 405, headers: CORS_HEADERS, body: JSON.stringify({ error: "GET only" }) };
  }
  ...
  try {
    ...
    if (!cached) {
      return {
        ...
        body: JSON.stringify({ error: `No cached Opportunity Intelligence data found for key "${cacheKey}". Has refresh-opportunity-intel.js been run yet?` }),
      };
    }
    ...
    if (params.player && params.pos) {
      ...
      if (!record) {
        return {
          statusCode: 404,
          headers: CORS_HEADERS,
          body: JSON.stringify({ error: `No record for key "${key}" in this window.`, computedAt: cached.computedAt, weeksRequested: cached.weeksRequested }),
        };
      }
      return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify(record, null, 2) };
    }

    return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify(cached, null, 2) };
  } catch (e) {
    return { statusCode: 500, headers: CORS_HEADERS, body: JSON.stringify({ error: "Cache read failed", detail: e.message }) };
  }
};
