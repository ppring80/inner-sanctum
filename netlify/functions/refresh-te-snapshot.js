// netlify/functions/refresh-te-snapshot.js
//
// WEEKLY SAGE — WR SNAPSHOT CACHE WRITER (Phase 1)
//
// PURPOSE
// -------
// Build the TE population snapshot for a given season/week/seasonType
// by calling weekly-sage-te-snapshot.js's buildTeSnapshot() IN PROCESS
...
// weekly-sage-te-leaderboard.js (once that file exists in a later phase). It exists to prove the cache writer
...
const { buildTeSnapshot } = require("./weekly-sage-te-snapshot.js");
...
/*
  ...
  guesses the TE population/eligibility rules themselves -- it only
...
function validateCompleteSnapshot(snapshot, { season, targetWeek, seasonType }) {
  ...
  if (snapshot.evidenceType !== "weekly-sage-te-snapshot") {
    problems.push(
...
exports.handler = async function (event) {
  ...
  const budgetError = await requireTank01Budget(event, { job: "refresh-te-snapshot", calls: 97 });
  ...
  */
  ...
  try {
    ...
    if (problems.length > 0) {
      console.error(
        `refresh-te-snapshot: build for ${key} was incomplete, NOT caching. Problems: ${problems.join(" | ")}`
      );
      ...
      return jsonResponse(422, {
        ...
        blobKey: key,
        error: "TE snapshot build was incomplete; existing cache (if any) was left untouched.",
        problems
      });
    }
    ...
    console.log(
      `refresh-te-snapshot: cached ${key} -- ${snapshot.population.length} eligible TE(s), 0 failures.`
    );
    ...
    return jsonResponse(200, {
      ...
      eligibleTEPopulation: snapshot.population.length,
      failures: snapshot.failures.length,
      blobStore: STORE_NAME,
      blobKey: key
    });
  } catch (error) {
    console.error(`refresh-te-snapshot failed for ${key}:`, error);
    ...
    return jsonResponse(502, {
      ...
      blobStore: STORE_NAME,
      blobKey: key,
      error: "Could not build Weekly SAGE TE snapshot.",
