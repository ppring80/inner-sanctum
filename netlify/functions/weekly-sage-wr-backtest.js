// netlify/functions/weekly-sage-wr-backtest.js
//
// WEEKLY SAGE — WR MULTI-WEEK BACKTEST
//
// PURPOSE
// -------
// Aggregate multiple historical Weekly SAGE WR validation weeks
...
// ------------------------
//
//   - combined WR player-week sample
//   - Pearson correlation
...
//   Weeks 5 through 17
//
// This mirrors the broad RB validation window and gives WR SAGE
...
async function fetchValidationWeek({
  ...
  if (
    ...
    return {
      ...
      error:
        "Unexpected Weekly SAGE WR validation schema."
    };
  }
...
/*
  ...
  One observation = one WR who actually played in that week.
*/
  ...
  async function (
    ...
    try {
      ...
      /*
        STEP 1
        ------
        Retrieve each weekly WR validation result.
      ...
      */
      ...
      /*
        STEP 2
        ------
        Build one clean WR player-week dataset.
      */
      const observations =
        [];
      ...
      */
      ...
      return jsonResponse(
        200,
        {
          ...
          methodology: {
            ...
            leakageProtection:
              "This endpoint consumes weekly-sage-wr-validation results. Target-week actual performance never feeds back into the historical SAGE prediction.",
            ...
            regression:
              "Simple ordinary least squares regression of actual fantasy points on pre-game WR SAGE Score.",
            ...
            important:
              "This backtest measures historical predictive relationship. It does not optimize WR weights or create recommendation thresholds."
          },
          ...
          nextStep: {
            ...
            reason:
              ready
                ? "The multi-week WR forecast-vs-actual dataset is clean. Review combined SAGE correlations, Role / Production / Matchup relationships, regression fit, and score-band calibration before running WR weight sensitivity."
                : "Resolve retrieval failures or unready validation weeks before interpreting the WR backtest or changing SAGE weights."
          },
      ...
      console.error(
        "weekly-sage-wr-backtest failed:",
        error
      );
      ...
      return jsonResponse(
        ...
        {
          error:
            "Could not build Weekly SAGE WR multi-week backtest.",
